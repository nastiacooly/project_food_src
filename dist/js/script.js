...
function calc() {
  //variables
  const calcResult = document.querySelector('.calculating__result span'); // Calculator
...
function forms(formSelector, modalSelector, modalTimer) {
  //variables
  const forms = document.querySelectorAll(formSelector); //Sending forms to server via Fetch API and showing status messages to user
...
/* harmony export */   "showStatusModal": function() { return /* binding */ showStatusModal; }
/* harmony export */ });
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, tabActiveClass) {
  ...
  const tabs = document.querySelectorAll(tabsSelector),
        ...
        tabsParent = document.querySelector(tabsParentSelector); //functions for tabs
...
function timer(timerSelector, endDate) {
  ...
  const millisecondsInASecond = 1000,
        ...
        millisecondsInAMinute = millisecondsInASecond * 60; //functions for timer
  ...
  const setTimer = (timerSelector, endDate) => {
    ...
    function updateTimer() {
      const t = getTimeRemaining(endDate); //puts numbers to HTML
